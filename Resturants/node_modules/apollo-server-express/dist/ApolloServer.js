...
const middlewareFromPath = (path, middleware) => (req, res, next) => {
    const parsedUrl = parseurl_1.default(req);
...
class ApolloServer extends apollo_server_core_1.ApolloServerBase {
    ...
    getMiddleware({ path, cors, bodyParserConfig, disableHealthCheck, onHealthCheck, } = {}) {
        ...
        if (!disableHealthCheck) {
            middleware.push(middlewareFromPath('/.well-known/apollo/server-health', (req, res) => {
        ...
        if (cors === true) {
            middleware.push(middlewareFromPath(path, cors_1.default()));
        }
        else if (cors !== false) {
            middleware.push(middlewareFromPath(path, cors_1.default(cors)));
        }
        if (bodyParserConfig === true) {
            middleware.push(middlewareFromPath(path, body_parser_1.json()));
        }
        else if (bodyParserConfig !== false) {
            middleware.push(middlewareFromPath(path, body_parser_1.json(bodyParserConfig)));
        }
        ...
        return compose_middleware_1.compose(middleware);
    }
}
